...
const findActiveFilename = parent => parent.querySelector('[data-file-active]');
...
const registerHandlebarsHelpers = () => {
  Handlebars.registerHelper('hasFailures', tests =>
    (tests || []).some(test => !test.pass)
  );
  Handlebars.registerHelper('getFailed', tests =>
    (tests || []).filter(test => !test.pass)
  );
  ...
  Handlebars.registerHelper('summariseLintIssues', lintIssues => {
    return (lintIssues || []).filter(x => x.ruleId).map(x => lintMap[x.ruleId]);
  });
  Handlebars.registerHelper('stringify', obj => Object.values(obj).join(' | '));
